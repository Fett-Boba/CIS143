...
/* Task
ISBN stands for International Standard Book Number.  For more than thirty years, ISBNs 
were 10 digits long. On January 1, 2007 the ISBN system switched to a 13-digit format. 
Now all ISBNs are 13-digits long. Actually, there is not a huge difference between them. 

You can convert a 10-digit ISBN to a 13-digit ISBN by adding the prefix number (978) 
...
public class Isbn10ToIsbn13 {

	  public static String isbnConverter(String isbn) {
		  
		  String isbn13 = "978-" + isbn.substring(0, 12); 
		  String digits = isbn13.replaceAll("[^0-9]", "");
		  int checkdigit = 0;
		  
		  for (int i = 0; i < digits.length(); i++) {
			  
			  checkdigit = (i % 2 == 0) ?  checkdigit + Character.getNumericValue(digits.charAt(i)) : checkdigit + 3 * (Character.getNumericValue(digits.charAt(i)));
			  
//			  if (  i % 2 == 0  ) { 						// if even digit add
//				  checkdigit = checkdigit + Character.getNumericValue(digits.charAt(i));
//			  } else { 										// odd digit, mult by 3 then add
//				  checkdigit = checkdigit + 3 * (Character.getNumericValue(digits.charAt(i)));
//			  }
		  ...
		  return   (checkdigit == 10) ?  isbn13 + "0"  :  isbn13 + checkdigit;
